...
| Semantic snapshot             |                pass |                pass | raw target id is reliable     |
...
The remaining interactive-action defect lives in OpenClaw `2026.8.2`'s persistent
Playwright session/target enumeration. It is reproducible with both browsers and
must be repaired or upgraded in the Gateway runtime; changing the extension to
mask it would violate the native-faithful deliverable's contract.
