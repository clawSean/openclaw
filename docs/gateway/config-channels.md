...
- `channels.slack.appHome.view` holds a display-only inline Slack Block Kit Home view object that replaces the built-in Home tab; edits apply through normal config reload. The view must be type `"home"` (or omit `type`, which OpenClaw fills as `"home"`) and contain 1-100 blocks. Action and input controls are rejected so configured presentation cannot enter Slack's interaction routing. Views that fail validation or that Slack rejects at publish time are logged and fall back to the built-in safe Home view. Account-level `channels.slack.accounts.<accountId>.appHome` overrides the top-level value for that account. Do not include secrets or private operational details; Slack shows the Home tab to workspace members who can open the app.
