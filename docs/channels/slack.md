...
To replace the default Home view, set `channels.slack.appHome.view` to a display-only inline Block Kit Home view object. Account-level `appHome` overrides the top-level value. Action and input controls are rejected so configured presentation cannot enter Slack's interaction routing. Views that fail validation or that Slack rejects at publish time are logged and fall back to the built-in default. Keep the view free of secrets because Slack shows Home-tab content to workspace members who can open the app.
