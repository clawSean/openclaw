...
`openclaw models status` shows the resolved default/fallbacks plus an auth overview. Active profile cooldowns appear under **Unavailable auth profiles** with the stored reason and recovery action; JSON output exposes the same data in `auth.unusableProfiles`. For plugin-owned agent runtimes such as Codex, status also checks whether the owning plugin is enabled and passed startup payload verification. A route with valid credentials but an unavailable runtime reports `status: unavailable` instead of `usable`; JSON output includes separate `authStatus`, `runtimeStatus`, and bounded runtime diagnostics. When provider usage snapshots are available, the **Usage-capable auth profiles** section includes usage windows and quota snapshots on each eligible configured auth profile. OAuth/token profiles are eligible by default; API-key profile fan-out is limited to the explicit ClawRouter and DeepSeek allowlist so a status read does not silently broaden provider traffic. Usage auth comes from provider-specific hooks when available; otherwise OpenClaw falls back to matching eligible credentials from auth profiles, env, or config.
