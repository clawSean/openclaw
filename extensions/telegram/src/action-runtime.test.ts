...
describe("handleTelegramAction", () => {
  ...
  it("does not expose a native copy value in a presentation-only message body", async () => {
    ...
    const call = mockCall(sendMessageTelegram, 0, "copy-only native send");
    expect(call[1]).toBe("Choose an option.");
  ...
  it("retains presentation text when a presentation-only copy action needs fallback", async () => {
    ...
    const call = mockCall(sendMessageTelegram, 0, "presentation-only degraded copy send");
    expect(call[1]).toBe(`Keep this context\n\n- Copy token: \`${copyValue}\``);
    expect(call[1]).not.toBe(copyValue);
    expect(
      requireRecord(call[2], "presentation-only degraded copy options").buttons,
    ).toBeUndefined();
    expect(resultDetails(result)).toMatchObject({
      ok: true,
      degradedDelivery: {
  ...
  it("edits reply markup with a native copy-text button", async () => {
    ...
    expect(editMessageTelegram).not.toHaveBeenCalled();
    const call = mockCall(editMessageReplyMarkupTelegram, 0, "native copy reply markup edit");
  ...
  it("renders presentation text in mixed copy-text edits without explicit content", async () => {
    ...
    const call = mockCall(editMessageTelegram, 0, "mixed presentation edit");
  ...
  it("preserves an explicit empty caption while editing native presentation buttons", async () => {
    ...
    expect(editMessageReplyMarkupTelegram).not.toHaveBeenCalled();
    const call = mockCall(editMessageTelegram, 0, "empty caption presentation edit");
    expect(call[2]).toBe("");
    expect(call[2]).not.toBe("Choose an option.");
  ...
  it("preserves callback-only partial success when one edited control is unencodable", async () => {
    ...
    expect(editMessageTelegram).not.toHaveBeenCalled();
    const call = mockCall(editMessageReplyMarkupTelegram, 0, "partial reply markup edit");
  ...
  it("does not replace the message body when invalid copy fallback needs explicit content", async () => {
    const result = await handleTelegramAction(
      {
        action: "editMessage",
        chatId: "123456",
  ...
  it("requires explicit content when the action budget displaces a copy action", async () => {
    const result = await handleTelegramAction(
      {
        action: "editMessage",
        chatId: "123456",
    ...
    expect(resultDetails(result)).toMatchObject({
      ...
      degradedDelivery: {
        droppedControls: 1,
        fallback: "not_delivered",
        reasons: ["presentation_action_budget_exceeded"],
        guidance: expect.stringContaining("explicit content or caption"),
      },
    });
  });
