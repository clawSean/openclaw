...
export function createTelegramMessageContextRuntime({
  ...
  pluginNativeCommandNames?: RegisterTelegramHandlerParams["pluginNativeCommandNames"];
}) {
  const ignoreEnabled =
    ...
    !hasTelegramCustomCommand({ commands: telegramCfg.customCommands, command: "ignore" }) &&
