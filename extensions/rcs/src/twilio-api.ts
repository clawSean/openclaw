...
function parseTwilioApiError(text: string): ParsedTwilioApiError {
  try {
    ...
    // SAFETY: JSON object fields are read only after individual runtime type checks.
...
function redactTwilioResponseText(text: string, account: ResolvedRcsAccount): string {
...
async function toTwilioApiResponse(
  response: Response,
...
export async function requestTwilioApi(params: {
  ...
  if (params.fetchImpl) {
    ...
    return await toTwilioApiResponse(response, params.account);
  }
